...
const WeeklySchedule = ({goToNextStep}: {
                            ...
                            return (
                                <div
                                    ...
                                    className={`bg-[#ffffff] border rounded-lg overflow-hidden transition-colors ${
                                        isSelected ? 'border-[#bd9554] shadow-sm shadow-[#bd9554]' : 'border-[#bd9554] hover:border-gray-300'
                                    }`}
                                    onClick={()=> setshowSlotAvailable((!showSlotAvailable))}
                                >
                                    <div
                                        className={`text-[#1e1e1e] flex cursor-pointer ${
                                            isSelected ? 'bg-gray-100' : 'hover:opacity-90'
                                        }`}
                                        onClick={() => !selectedDay ? setSelectedDay(dayInfo) : setSelectedDay(null)}
                                    >
                                        ...
                                        <div
                                            ...
                                            <div className='w-1/2 ml-2'>
                                                <h3 className="font-bold text-lg">
                                                    {format(dayInfo.fullDate, 'EEEE', {locale: es}).toUpperCase()}
                                                </h3>
                                                <p className="text-sm text-[#1e1e1e] font-medium">
                                                    {format(dayInfo.fullDate, 'd MMMM', {locale: es})}
                                                </p>
                                            </div>
                                            <div className='w-1/2'>
                                                {daySlots.length > 0 ? (
                                    ...
                                    <AnimatePresence>
                                        {isSelected && showSlotAvailable && daySlots.length > 0 && (
                                            <motion.div
                                      ...
                                      {appt.status === 'disponible' ? 'Disponible' :
                                          appt.status === 'reservado' ? 'Reservado' :
                                              appt.status === 'completado' ? 'Completado' : 'Cancelado'}
                                                                        ...
                                                                        <button onClick={() => goToNextStep(appt, 2)} className="bg-[#1e1e1e] text-white px-2 py-1 rounded-md hover:opacity-70 transition-colors">
                                                                            Reservar
                                                                        </button>
                                                                    </div>
                                                                </li>
